...
import { TrendingDown, AlertCircle, CheckCircle2, Clock, Plus, Trash2, Download, FileText, BarChart3, CreditCard, Copy, Check, Pencil } from 'lucide-react';

const STORAGE_KEY = 'poison_donuts_v1';
...
const formatBoleto = (codigo) => {
  ...
  if (clean.length === 48) {
    return `${clean.slice(0,11)} ${clean.slice(11,23)} ${clean.slice(23,35)} ${clean.slice(35)}`;
  }
  return clean;
};

// ---------------------------------------------------------------------------
// Camada de persistência — usa localStorage.
// Para migrar para um backend real, substitua estas duas funções por chamadas
// fetch() à sua API e remova a dependência do localStorage.
// ---------------------------------------------------------------------------
const storage = {
  load: () => {
    try {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      const categoriasSalvas = saved.categorias || [];
      const categoriasMescladas = [
        ...categoriasSalvas,
        ...DEFAULT_DATA.categorias.filter(c => !categoriasSalvas.includes(c)),
      ];
      setData({ ...DEFAULT_DATA, ...saved, categorias: categoriasMescladas });
    }
    setLoading(false);
  }, []);
  ...
  const despesasFiltradas = useMemo(() => {
    return data.despesas.filter(d => d.data.startsWith(filtroMes));
  }, [data.despesas, filtroMes]);

  const totalMes = useMemo(() => despesasFiltradas.reduce((s, d) => s + Number(d.valor), 0), [despesasFiltradas]);
  const contasAbertas = useMemo(() => data.contas.filter(c => !c.pago), [data.contas]);
    ...
    <div className="app">
      ...
      <header className="header">
        <div className="brand">
          ...
          <div>
            <h1>POISON DONUTS</h1>
            <p>Financeiro · Ipanema · <span className={saving ? 'sync-saving' : 'sync-ok'}>{saving ? 'salvando…' : 'sincronizado'}</span></p>
          </div>
        </div>
      ...
      <main className="main">
        {tab === 'dashboard' && <Dashboard data={data} despesasFiltradas={despesasFiltradas} totalMes={totalMes} contasAbertas={contasAbertas} filtroMes={filtroMes} />}
        {tab === 'despesas' && <Despesas data={data} persist={persist} despesasFiltradas={despesasFiltradas} totalMes={totalMes} filtroMes={filtroMes} />}
        {tab === 'contas' && <Contas data={data} persist={persist} />}
      </main>
    </div>
  );
}

function Dashboard({ data, despesasFiltradas, totalMes, contasAbertas, filtroMes }) {
  ...
  const evolucaoDiaria = useMemo(() => {
    ...
    despesasFiltradas.forEach(d => {
      ...
      days[dia] = (days[dia] || 0) + Number(d.valor);
    });
...
function Despesas({ data, persist, despesasFiltradas, totalMes, filtroMes }) {
  ...
  const add = () => {
    ...
    const novo = { ...form, id: uid(), valor: Number(form.valor) };
    persist({ ...data, despesas: [novo, ...data.despesas] });
    setForm({ ...form, descricao: '', valor: '', fornecedor: '', observacao: '' });
  };

  const confirmarRemocao = () => {
    if (!confirmar) return;
    persist({ ...data, despesas: data.despesas.filter(d => d.id !== confirmar.id) });
    setConfirmar(null);
  };
  ...
  const exportCSV = () => {
    ...
    const rows = despesasFiltradas.map(d =>
      [d.data, `"${d.descricao}"`, d.categoria, `"${d.fornecedor || ''}"`, d.formaPagamento, d.valor.toFixed(2).replace('.', ','), `"${d.observacao || ''}"`].join(';')
    );
    ...
    a.href = url;
    a.download = `poison_despesas_${filtroMes}.csv`;
    a.click();
  };
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 16 }}>
          ...
          {despesasFiltradas.length > 0 && (
            <button className="btn-soft" onClick={exportCSV}><Download size={14} /> CSV</button>
          )}
        </div>
        {despesasFiltradas.length === 0 ? (
                ...
                {despesasFiltradas.map(d => (
                  <tr key={d.id}>
                    ...
                    <td>
                      <div>
                        {d.descricao}
                        {d.contaOrigemId && <span className="auto-tag" title="Gerada automaticamente ao marcar conta como paga">🔗 boleto</span>}
                      </div>
                    ...
                    <td>
                      <button className="icon-btn" onClick={() => setConfirmar(d)} title="Excluir"><Trash2 size={15} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="modal-overlay" onClick={() => setConfirmar(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p className="modal-text">
              <strong>{confirmar.descricao}</strong><br />
              <span className="muted">{formatDate(confirmar.data)} · {formatBRL(confirmar.valor)}</span>
            </p>
            ...
            <div className="modal-actions">
              ...
              <button className="btn-danger" onClick={confirmarRemocao}>
...
function Contas({ data, persist }) {
  ...
  const add = () => {
    ...
    const novo = { ...form, id: uid(), valor: Number(form.valor), pago: false, codigoBarras: form.codigoBarras.replace(/\D/g, '') };
    persist({ ...data, contas: [novo, ...data.contas] });
  ...
  const confirmarPagamento = () => {
    ...
    const despesaVinculada = {
      ...
      formaPagamento: conta.codigoBarras ? 'Boleto' : 'PIX',
      observacao: conta.observacao || '',
      contaOrigemId: conta.id,
    };
    persist({
      ...data,
      contas: data.contas.map(c => c.id === conta.id ? { ...c, pago: true, dataPagamento: dataPag, despesaVinculadaId: despesaVinculada.id } : c),
      despesas: [despesaVinculada, ...data.despesas],
    });
    setPagarModal(null);
  };

  const reabrirConta = (id) => {
    const conta = data.contas.find(c => c.id === id);
    if (!conta) return;
    persist({
      ...data,
      contas: data.contas.map(c => c.id === id ? { ...c, pago: false, dataPagamento: null, despesaVinculadaId: null } : c),
      despesas: conta.despesaVinculadaId ? data.despesas.filter(d => d.id !== conta.despesaVinculadaId) : data.despesas,
    });
  };
  ...
  const salvarEdicao = () => {
    ...
    const contaAtualizada = {
      ...
      codigoBarras: (editando.codigoBarras || '').replace(/\D/g, ''),
    };
    let despesasAtualizadas = data.despesas;
    if (contaAtualizada.pago && contaAtualizada.despesaVinculadaId) {
      despesasAtualizadas = data.despesas.map(d => {
        if (d.id === contaAtualizada.despesaVinculadaId) {
          return {
            ...d,
            data: contaAtualizada.dataPagamento || d.data,
            descricao: contaAtualizada.descricao,
            categoria: contaAtualizada.categoria || 'Outras',
            valor: Number(contaAtualizada.valor),
            fornecedor: contaAtualizada.fornecedor || '',
            observacao: contaAtualizada.observacao || '',
          };
        }
        return d;
      });
    ...
    persist({
      ...data,
      contas: data.contas.map(c => c.id === contaAtualizada.id ? contaAtualizada : c),
  ...
  const confirmarRemocao = () => {
    if (!confirmar) return;
    persist({ ...data, contas: data.contas.filter(c => c.id !== confirmar.id) });
    setConfirmar(null);
  };
  ...
  const abertas = data.contas.filter(c => !c.pago).sort((a, b) => a.vencimento.localeCompare(b.vencimento));
  const pagas = data.contas.filter(c => c.pago).sort((a, b) => (b.dataPagamento || '').localeCompare(a.dataPagamento || ''));
  ...
  return (
    <div>
        ...
        <div className="form-grid">
          ...
          <div className="form-field flex-full">
            ...
            <input
              ...
              placeholder="Cole aqui o código do boleto (47 ou 48 dígitos)"
              className="mono"
            />
            {form.codigoBarras && (
              <div className="boleto-preview">{formatBoleto(form.codigoBarras)}</div>
            )}
          </div>
          <div className="form-field flex-2">
            <label>Observação</label>
      ...
      <div className="card">
        <h3>Em aberto ({abertas.length}) — {formatBRL(abertas.reduce((s, c) => s + c.valor, 0))}</h3>
        {abertas.length === 0 ? (
                ...
                <div key={c.id} className={`conta-card ${vencida ? 'conta-vencida' : proxima ? 'conta-proxima' : ''}`}>
                  ...
                  {c.codigoBarras && (
                    <div className="boleto-row">
                      ...
                      <button className="btn-copy" onClick={() => copiarBoleto(c.codigoBarras, c.id)} title="Copiar código">
                  ...
                  <div className="conta-actions">
                    <button className="btn-pay" onClick={() => abrirModalPagamento(c)}>
                      <CheckCircle2 size={14} /> Marcar como paga
                    </button>
                    <button className="btn-edit" onClick={() => setEditando({ ...c })}>
              ...
              <tbody>
                {pagas.slice(0, 30).map(c => {
                  ...
                  return (
                    <tr key={c.id}>
                      <td>
                        <strong>{formatDate(c.dataPagamento || c.vencimento)}</strong>
                        {atrasado && <div className="muted-warn">após vencimento</div>}
                      </td>
                      <td>
                        <div>{c.descricao}</div>
                        {c.fornecedor && <div className="muted">{c.fornecedor}</div>}
        ...
        <div className="modal-overlay" onClick={() => setEditando(null)}>
          <div className="modal modal-wide" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-grid">
              <div className="form-field flex-full">
                <label>Descrição</label>
                <input type="text" value={editando.descricao || ''} onChange={e => setEditando({ ...editando, descricao: e.target.value })} />
              </div>
              <div className="form-field">
                <label>Fornecedor</label>
                <input type="text" value={editando.fornecedor || ''} onChange={e => setEditando({ ...editando, fornecedor: e.target.value })} />
              </div>
              <div className="form-field">
                <label>Categoria</label>
              ...
              <div className="form-field">
                <label>Data de emissão</label>
                <input type="date" value={editando.dataEmissao || ''} onChange={e => setEditando({ ...editando, dataEmissao: e.target.value })} />
              </div>
              <div className="form-field">
                <label>Vencimento</label>
                <input type="date" value={editando.vencimento || ''} onChange={e => setEditando({ ...editando, vencimento: e.target.value })} />
              </div>
              {editando.pago && (
                <div className="form-field">
                  <label>Data do pagamento</label>
                  <input type="date" value={editando.dataPagamento || ''} onChange={e => setEditando({ ...editando, dataPagamento: e.target.value })} />
                </div>
              )}
              <div className="form-field flex-full">
                <label>Código de barras (boleto)</label>
                ...
                />
                {editando.codigoBarras && (
                  <div className="boleto-preview">{formatBoleto(editando.codigoBarras)}</div>
                )}
              </div>
              <div className="form-field flex-full">
                <label>Observação</label>
                <input type="text" value={editando.observacao || ''} onChange={e => setEditando({ ...editando, observacao: e.target.value })} />
              </div>
            </div>
            {editando.pago && editando.despesaVinculadaId && (
              <p className="modal-info" style={{ marginTop: 16 }}>
                Esta conta está paga. A despesa vinculada será atualizada automaticamente.
              </p>
            )}
      ...
      {pagarModal && (
        <div className="modal-overlay" onClick={() => setPagarModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p className="modal-text">
              <strong>{pagarModal.descricao}</strong><br />
              <span className="muted">Vencimento: {formatDate(pagarModal.vencimento)} · {formatBRL(pagarModal.valor)}</span>
            </p>
            <div className="form-field" style={{ marginBottom: 16 }}>
              ...
              />
              {dataPagamentoInput && pagarModal.vencimento && dataPagamentoInput > pagarModal.vencimento && (
                <div className="muted-warn" style={{ marginTop: 6 }}>
                  ⚠ Pagamento após o vencimento
                </div>
              )}
            </div>
            <p className="modal-info">
              Uma despesa será criada automaticamente na data informada.
            </p>
            <div className="modal-actions">
              <button className="btn-soft" onClick={() => setPagarModal(null)}>Cancelar</button>
              <button className="btn-pay" onClick={confirmarPagamento}>
      ...
      {confirmar && (
        <div className="modal-overlay" onClick={() => setConfirmar(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p className="modal-text">
              <strong>{confirmar.descricao}</strong><br />
              <span className="muted">Venc.: {formatDate(confirmar.vencimento)} · {formatBRL(confirmar.valor)}</span>
            </p>
            <p className="modal-warn">Esta ação não pode ser desfeita.</p>
            <div className="modal-actions">
              <button className="btn-soft" onClick={() => setConfirmar(null)}>Cancelar</button>
...
const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .donut-ring {
    ...
    border: 8px solid #2a1a24;
    border-top-color: #ff3d8a;
    border-radius: 50%;
  ...
  .header {
    ...
    align-items: center;
    padding-bottom: 24px;
    border-bottom: 1px solid #2a1a24;
    margin-bottom: 24px;
    flex-wrap: wrap;
    gap: 16px;
  }
  ...
  .header h1 {
    ...
    text-shadow: 0 0 20px rgba(255, 61, 138, 0.3);
  }
  ...
  .tab {
    ...
    padding: 12px 18px;
    font-family: 'Bricolage Grotesque', sans-serif;
    font-size: 13px; color: #8a7080; cursor: pointer;
  ...
  .card h3 {
    font-family: 'Bricolage Grotesque', sans-serif;
    font-weight: 700; font-size: 16px; color: #f4e8ee;
  ...
  .kpi {
    background: rgba(255, 255, 255, 0.025);
    border: 1px solid #2a1a24; border-radius: 14px;
    padding: 20px; display: flex; gap: 14px; align-items: flex-start;
  }
  .kpi-main {
    background: linear-gradient(135deg, rgba(255, 61, 138, 0.18) 0%, rgba(255, 61, 138, 0.05) 100%);
    border-color: #ff3d8a; flex-direction: column; align-items: flex-start;
    box-shadow: 0 0 30px rgba(255, 61, 138, 0.15);
  }
  .kpi-icon {
    width: 36px; height: 36px; background: rgba(255, 61, 138, 0.15);
    border-radius: 8px; display: flex; align-items: center; justify-content: center;
    color: #ff3d8a; flex-shrink: 0;
  }
  ...
  .form-field input, .form-field select {
    padding: 10px 12px; border: 1px solid #3a2a34; border-radius: 7px;
    background: rgba(0, 0, 0, 0.3); font-family: inherit;
    font-size: 13px; color: #f4e8ee; color-scheme: dark;
  }
